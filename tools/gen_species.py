...
def make_metering_inlet_textures(rgb):
    """B4 metering inlet face set (see gen_block_textures for orientation).

    The inward face is a simple spout; the outward face is a valved pipe
    coupling with a small flow-scale window so the metering role reads from
    the outside.
    """
    r, g, b = (rgb >> 16) & 0xFF, (rgb >> 8) & 0xFF, rgb & 0xFF
    front = []
    ...
    for y in range(16):
        ...
        back_row = []
        for x in range(16):
            # Internal face: a rounded nozzle opening flush with the wall.
            if (4 <= x <= 11) and (5 <= y <= 10):
                if (6 <= x <= 9) and (7 <= y <= 8):
                    front_row += [22, 26, 32, 255]        # spout bore
                else:
                    front_row += [150, 158, 168, 255]     # nozzle ring
            else:
                front_row += [r, g, b, 255]
            # External face: pipe coupling bore + valve handwheel + scale ticks.
            dx, dy = x - 7.5, y - 7.5
            d2 = dx * dx + dy * dy
            if d2 <= 20:
                if d2 <= 9:
                    ...
                    back_row += [176, 183, 190, 255]       # coupling ring
            elif y in (3, 4) and x in (3, 4, 11, 12):
                back_row += [235, 200, 60, 255]            # valve handwheel
            elif y in (12, 13) and 4 <= x <= 11 and (x - 4) % 2 == 0:
                back_row += [240, 240, 240, 255]           # scale ticks
            else:
                back_row += [r, g, b, 255]
        front.append(front_row)
        back.append(back_row)
...
def gen_block_textures():
    ...
    # B4 metering inlet: FACING points into the vessel. The inward face is a
    # nozzle spout, the outward face is the valved metering coupling (the
    # sole liquid inlet), the other four are casing.
