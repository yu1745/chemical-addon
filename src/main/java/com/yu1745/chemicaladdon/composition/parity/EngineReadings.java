 ...
 * （JNA 每次求解 ~1ms 量级，六种表计 × 多釜会叠加）。本类在釜求解后缓存一次
 * 结果，表计共享读取；读取侧无缓存时回退 legacy 读数（渐进迁移，开关见
 * {@code ChemEngineConfig}）。
 *
 * <p>生命周期：ReactorController tickReaction 双跑阶段由 PARITY_OBSERVE 驱动；
 * 读数源切换后由 tickReaction 每次求解后 refresh。线程模型：MC 服务端单线程
 * tick，无需并发控制。
 */
public final class EngineReadings {
	...
	/** 从釜流体求解并缓存（tickReaction 调用）。失败/空釜 → INVALID（读取侧回退 legacy）。 */
